...
GLFWwindow* window;
Camera* camera;
GLuint particleShaderProgram, normalShaderProgram, fresnelShaderProgram;
...
float height_threshold = 6000.0f;
...
void createContext() {
	...
	/*fresnelShaderProgram = loadShaders(
...
void mainLoop() {
	...
	OrbitEmitter cloud_emitter = OrbitEmitter(cloud, 100, 2, 2);
	...
	do {
		...
		//cloud_emitter.renderParticles(); //UNCOMMENT
...
void initialize() {
	...
	glDisable(GL_CULL_FACE);					//by default CCW
...
void pollKeyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	//More Wind in Z Axis - U - affecting the clouds as well
